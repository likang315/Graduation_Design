			...
			<div class="myDivB">
				<table>
						...
						<tr>
							...
							<td colspan="2">
								<a onclick = "tiaozhuan('${map.channel_code}','${map.id}')">${map.id}</a>
							</td>
						</tr>
						<tr>
							...
							<td>${map.courierPhone}</td>
						</tr>
						<tr>
							<td><label>快递员姓名:</label></td>
							<td>${map.courierName}</td>
...
<script type="text/javascript">
   ...
   $("#xuanze").change(function(){
		...
		if(myseach(startTime, endTime)){
			var parameter = {"channel_code":channel_code, "searchMsg":searchMsg, "state":state, "startTime":startTime, "endTime":endTime,"jiazai":jiazai};
			appendTable(parameter);
			}else {
	...
	/* function  appendTable(channel_code,searchMsg,state,startTime,endTime){
		
		window.location.href="${pageContext.request.contextPath}/app/storeSupplies/storeList.html?channel_code="+channel_code+"&&searchMsg="+searchMsg+"&&state="+state
