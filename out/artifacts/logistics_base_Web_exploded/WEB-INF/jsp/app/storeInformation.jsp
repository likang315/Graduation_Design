...
<html>
	<head>
		...
		<style type="text/css">
			.garden1{color:#bf4c28; font-weight:bold; border-bottom:2px solid #bf4c28;}
			.divGarden1{color:#bf4c28;border:2px solid #bf4c28; }
		</style>
		<script type="text/javascript">
		$(document).ready(function(){
			API.checkLogin();
			$.post("${pageContext.request.contextPath}/app/fillingSingle/getVendor.html",function(data1){
				/* alert(data1.vendor.length);
			...
			$("#vendor").change(function(){
				...
				$.post("${pageContext.request.contextPath}/app/fillingSingle/getStoreById.html",{"vendor_id":vendor_id, "type":type},function(data1){
					if(data1.store.length>0){
						$("#info").html("");
						for (var i = 0; i < data1.store.length; i++) {
							str="<div  class='store' name='material' data='"+data1.store[i].channel_code+"' onclick='dianji(\""+data1.store[i].store_name+"\",this)'>"+data1.store[i].store_name+"</div>";
			...
			$("#mohuchaxun").keyup(function(){
				...
				var type = $("#type").val();//门店类别(直营/现金)
				 $.post("${pageContext.request.contextPath}/app/fillingSingle/getStoreByMohuchaxun.html",{"vendor":vendor,"mohuchaxun":mohuchaxun, "type":type},function(data1){
					/* var data=JSON.parse(data1); */
					if(data1.mohu.length>0){
						$("#info").html("");
						for (var i = 0; i < data1.mohu.length; i++) {
							/*修改时间：2017年11月22日10:54*/
							/*修改原因：没有给data赋值，模糊查询后无法将门店传递到下一个页面去*/
							/*修改前：str="<div class='store' name='material' onclick='dianji(\""+data1.mohu[i].store_name+"\",this)'>"+data1.mohu[i].store_name+"</div>";*/
		...
		<div class="container">
		    <div class="procedure">
		    	<div class="col-xs-4">
		            ...
		            <a><div class="garden1">门店信息</div></a>
		    	</div>
		        <div class="col-xs-4">
			...
			<form id="form" action="${pageContext.servletContext.contextPath }/app/fillingSingle/toMaterialInformation.html" method="post">
			   <div class="selectDiv">
