		...
		<%-- <c:choose>
			<c:when test="${not empty map}">
				<c:forEach  items="${map}"  var="map" >
					...
					<div class="myDivB"  onclick = "tiaozhuan('${map.channel_code}','${map.id}')">
						<table>
							<tr>
									<td colspan="2"><label>订单编号:</label></td>
									<td colspan="2">
										${map.id}
									</td>
							</tr>
							...
							<tr>
								...
								<td style="color: green;" ><c:if test="${map.state==0 or map.state==1 or map.state==2 or map.state==3}"> 已接单</c:if><c:if test="${map.state==4}"> 配送中</c:if><c:if test="${map.state==5 or map.state==6}">配送成功</c:if></td>
							</tr>
						</table>
					</div>
   ...
   var user=API.getUserInfo();
	...
	$("#xuanze").change(function(){
		...
		if(myseach(startTime, endTime)){
			var parameter = {"accountName":accountName, "searchMsg":searchMsg, "state":state, "startTime":startTime, "endTime":endTime,"jiazai":jiazai};
			appendTable(parameter);
		}else {
...
/*
function  appendTable(accountName,searchMsg,state,startTime,endTime,jiazai){
		if(size>jiazai){
			alert(size)
			alert(jiazai)
			$("#dianjigengduo").show();
			$("#dianjigengduo").html("加载更多");
		}else{
			$("#dianjigengduo").hide();
			$("#dianjigengduo").html("");
		}
		window.location.href="${pageContext.request.contextPath}/app/coordinate/CourierAllList.html?accountName="+accountName+"&&searchMsg="+searchMsg+"&&state="+state
		+"&&startTime="+startTime+"&&endTime="+endTime+"&&jiazai="+jiazai  ;
