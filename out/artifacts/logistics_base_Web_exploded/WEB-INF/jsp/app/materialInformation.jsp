...
<head>
	...
	<style type="text/css">
		.garden2{color:#bf4c28; font-wei${pageContext.request.contextPath}ght:bold; border-bottom:2px solid #bf4c28;}
		.divGarden2{color:#bf4c28;border:2px solid #bf4c28; }
	</style>
</head>
	...
	<!-- header -->
	    ...
	    <div class="labelInformation">物流信息</div>
	  	<div class="tableDivFillIn">
		  	<form id="form" action="${pageContext.servletContext.contextPath }/app/fillingSingle/toCourier.html" method="post">
		   		<input type="hidden" id="store" name="store" value="${store }"/>
		        <table border="1" bordercolor="#ccc" width="100%">
<!-- 		        	<tr>
		            	<td>物流单号:</td>
		                <td><input type="text" id="logistics" name="logistics" class="logisticsText"></td>
		            </tr> -->
		            <tr>
		            	<td width="30%">预计时长:</td>
		                <td>
		                	<input type="number"  min="0" id="predictTime" name="predictTime" style="width:30%; height:27px; border-radius:3px; border:1px solid #CCC;padding-left: 5px;">
		                	小时(选填)
		                </td>
		            </tr>
		            <tr>
		            ...
		            <tr>
		            	<td>物品数量:</td>
		                <td><input type="number" min="0" id="materialNumber" id="materialNumber" style="width:30%; height:27px; border-radius:3px; border:1px solid #CCC; padding-left: 5px;"/></td>
		            </tr>
		        </table>
		        ...
		        <div class="addToDiv">
		        	...
		        	<input type="button" id="nextStep" value="下一步" class="btn btn-success btn-xs" style="width:100px; margin-top:5px;">
		        </div>
		   	</form>
		</div>
	</div>
...
<script>
	...
	$("#ok").click(function(){
		...
		/* 修改时间：2017年4月12日11:05:59 殷瑜泰
		修改原因： bug修复，防止负数提交
		修改前代码： var num= Number($("#materialNumber").val());//物资数量
				var predictTime = Number($("#predictTime").val());//预计时长 */
		var num= Number($("#materialNumber").val());//物资数量
		var predictTime = Number($("#predictTime").val());//预计时长
		/* 新增原因：2017年4月12日10:23:28 殷瑜泰
		新增原因：防止非法数据提交 */
		/* if(predictTime <= 0){
			$("#mesg").html("请输入合法的时间！");
			$("#mesg").fadeIn(500);
		...
		var data = {"num":num, "material":material, "materialName":materialName, "store":store, "operation":"append", "predictTime":predictTime};
		if(check()){
	...
	function mydelete(materialId){
		$.post("${pageContext.request.contextPath}/app/fillingSingle/appendOrRemove.html", {"material":materialId, "operation":"remove"}).done(function(data){
			$("#mytbody").html("");
			var mytbody;
			for(var i = 0; i < data.length; i++){
				mytbody += "<tr>"
