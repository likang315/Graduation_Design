...
<html>
	<head>
       ...
       <script type="text/javascript" src="http://api.map.baidu.com/library/SearchInfoWindow/1.5/src/SearchInfoWindow_min.js"></script>
		<style type="text/css">
			/* 本页面css样式 */
			 .a { position:fixed; left:40%; width: 25%;margin-top: 10px; }
	...
	<!-- header -->
        ...
        <input type="button"  id="peisong" onclick="peisong('${maps.store_longitude}','${maps.store_latitude}','${maps.courierPhone}','${maps.id}','${maps.address }','${maps.channel_code }','${maps.phone}','${maps.consignee }')"
                 class="btn btn-success" value="<c:if test='${maps.state=="4"}'>查看派送地图</c:if><c:if test='${maps.state=="0" or maps.state=="1"or maps.state=="2"}'>开始派送</c:if>" 
                 id="peisong" />
         </div>
         </c:if> 
	...
	function peisong(store_longitude,store_latitude,courierPhone,id,address,channel_code,phone,consignee){
		var peisong=$("#peisong").val()
			API.ajax("/app/coordinate/updateState",{"id":id}).success(function(datas){
			     if(datas.state =="ok" ){
			    	 if(peisong=="开始派送"){
			    		 API.ajax("/app/coordinate/addCourierstore",{"courier_longitude":lng,"courier_latitude":lat,"courier_Phone":courierPhone,"logistics":id}).success(function(data){
					 			if(data.state="ok"){
			    		 ...
			    		 API.ajax("/app/coordinate/updateCourierstore",{"courier_longitude":lng,"courier_latitude":lat,"courier_Phone":courierPhone,"logistics":id}).success(function(data){
					 			if(data.state="ok"){
