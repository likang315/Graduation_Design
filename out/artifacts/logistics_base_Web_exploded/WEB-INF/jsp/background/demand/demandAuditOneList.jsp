	...
	<div id="usual1" class="usual">
		...
		<div class="line">
			<div id="tab2" class="tabson">
				...
				<span id="myResult" style="display: none; font-size: 18px; color: red; text-align: center;">${result }</span>
				<form id="myform" name="fenye" action="${pageContext.servletContext.contextPath }/background/DemandAuditOne/getList.html" method="post">
					<div style="font-size: 12px; width: 100%; text-align: right; margin-bottom: 10px;">
						选择审核状态:
 						<select id="audit" name="dataState" style="border:1px solid #000;opacity:1;height: 35px;width: 115px; border-radius:5px">
 							...
 							<c:if test="${dataState == '3' }">
 								<option value="all" >全部</option>
 								<option value="0">待审核</option>
								<option value="3" selected="selected">审核通过</option>
								<option value="4">审核驳回</option>
 							</c:if>
 							<c:if test="${dataState == '4' }">
 								<option value="all">全部</option>
 								<option value="0">待审核</option>
								<option value="3">审核通过</option>
								<option value="4" selected="selected">审核驳回</option>
 							</c:if>
						</select>
			  			选择开始日期:
						...
						<c:if test="${dataState == '0' }">
							<button type="button" id="auditBtn" class="btn btn-primary" data-toggle="modal" data-target="#myModal">审核</button>
						</c:if>
						<c:if test="${dataState == '3' }">
					...
					<table class="tablelist">
						<thead>
							<tr>
								...
								<!-- <th>扩展需求</th> -->
								<th>审核状态</th>
								<%-- <c:if test="${dataState == '4' || dataState == 'all'}">
									<th>驳回原因</th>
								</c:if> --%>
								
								<!-- 新增时间：2017年4月12日14:10:46 殷瑜泰
								新增原因：方便列表直观操作 -->
									<th>操作</th>
								...
								<c:when test="${not empty ls}">
									<c:forEach items="${ls}" var="auditData" varStatus="start">
										<tr >
											<c:if test="${dataState == '0'}">
												<td>
													<input type="checkbox" id="check" name="checkS" value="${auditData.id }" />
												</td>
											</c:if>
											<td>${start.index+1 }</td>
											<td  onclick='show(${auditData})'><a>${auditData.store_name }</a></td>
											<%-- <td>${auditData.channel_code }</td> --%>
											<td>${auditData.store_shopowner_phone }</td>
											<td>${auditData.store_shopowner_name }</td>
											<td>${auditData.materialContent }</td>
											<%-- <td>${auditData.materialNumber }</td> --%>
											<td><fmt:formatDate value="${auditData.report_time }" pattern="yyyy-MM-dd HH:mm" /></td>
											<%-- <td>
												<c:choose>
													<c:when test="${not empty auditData.expanding_demand }">
														${auditData.expanding_demand }
													</c:when>
													<c:otherwise>
														无
													</c:otherwise>
											...
											<td >
												...
												<c:if test="${auditData.examine == 1 }">
													...
													<span style="color: green;" >通过(营销中心)</span> 
												</c:if>
												<c:if test="${auditData.examine == 2 }">
													
													<span style="color: red;" >驳回(营销中心)</span> 
												</c:if>
												<c:if test="${auditData.examine == 3 }">
												<span style="color: green;" >通过(渠道经理)</span> 
												</c:if>
												<c:if test="${auditData.examine == 4 }">
												<span style="color: red;" >驳回(渠道经理)</span> 
											...
											<td>
													<c:choose>
														<c:when test="${not empty auditData.examine_reason }">
															${auditData.examine_reason }
														</c:when>
														<c:otherwise>
															无
														</c:otherwise>
											...
											<!-- 新增时间：2017年4月12日14:28:23 殷瑜泰
												   新增原因：方便列表直观操作 -->
											<c:if test="${auditData.examine == 0 }">
											<td>
											<a href="javascript:void(0);" onclick="audit(${auditData.id })" data-toggle="modal" data-target="#myModal" >审核</a>
											</td>
											</c:if>
											<c:if test="${auditData.examine == 1 ||auditData.examine ==2 ||auditData.examine == 3 || auditData.examine == 4}">
											<td>
											暂无操作
											</td>
											</c:if>
	...
	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-body">
				<form id="NotgrantForm" action="${pageContext.request.contextPath }/background/DemandAuditOne/updateAuditData.html" method="post">
					<input type="hidden" id="checkData" name="checkDatas" />
					<table style="margin: auto;">
						<tr style="height:50px;">
							<td>请选择审核结果: &nbsp;&nbsp;&nbsp;</td>
							<td>
								<select id="Notgrant" name="examine" style="border:1px solid #ccc;opacity:1;height: 35px;width: 150px; border-radius:3px">
						...
						<tr style="display: none;" id="myExamine_reasontd">
							<td>请输入驳回原因:&nbsp;&nbsp;&nbsp;</td>
							<td>
								<textarea rows="5" placeholder="在这里输入驳回原因" style="border: 1px #ccc solid;width: 150px;resize:none; border-radius:3px" id="myExamine_reason" name="examine_reason"></textarea>
							</td>
						</tr>
					</table>
				</form>
...
$("#mengban").click(function(){
	if($("#Notgrant").val() != "pleaseSelect"){
		if($("#Notgrant").val() === "4" && $("#myExamine_reason").val() === ""){
			alert("请输入驳回原因");
			return false;
		}
		$("#NotgrantForm").submit();
...
$("#exportData").click(function(){
	var startTime = $("#startTime").val();
	var endTime = $("#endTime").val();
	location.href="${pageContext.request.contextPath }/background/DemandAuditOne/exportData.html?startTime="+startTime+"&endTime="+endTime;	
});
