...
<style type="text/css">
	...
	.table-striped tr td:first-child{text-align: right;}
</style>	
</head>
...
<div style="background:#000;position:fixed;top:0;left:0; width:100%; height:100%; opacity:0.2; z-index:9999;display:none; " id="modalmb2"></div>
	...
	<form id="sendForm" action="${pageContext.request.contextPath }/background/order/sendOrder.html" method="post">
		...
		<div class="modal-content">
			...
			请选择快递公司：
				...
				<input type="checkbox" name="seleCompany" value="${l.id }" onclick="getCourier()"/>${l.company_name }
				</c:forEach>
				<hr/>
...
<div style="background:#000;position:fixed;top:0;left:0; width:100%; height:100%; opacity:0.2; z-index:9999;display:none; " id="modalmb1"></div>
<div style="z-index:9999;position:fixed;top:100px; width:100%; display:none;" id="modalnr1">
		...
		<input id="isSeach1" type="hidden" value="${state }"/>
			<div class="toolbar1" id="toolDiv">
			    <div class="input-group pull-right" style="width:400px;">
		                        <input type="button" id="shanchu" class="btn btn-success"  value="批量删除" style="<c:if test='${state!=1 }'>display:none</c:if>"/>
		                        <input type="button" id="peisong" class="btn btn-success"  value="批量派送" style="<c:if test='${state!=1 }'>display:none</c:if>"/>
		                        
		                    </div>
				<div class="input-group pull-right" style="width:300px;">
		                        <input type="text" id="searchPhone" class="form-control" placeholder="请输入电话号码或订单号"   style="width: 200px;"  value="${phone }"/>
				...
				<ul class="input-group pull-right">
					<c:if test="${state== 3 }">
					<input id="download1" style="background-color: #3EB0D5;color:white; width:90px;height: 35px;" type="button" value="下载清单" />
			    				</c:if>
			...
			<div class="toolbar1" id="toolDiv1" style="display: none; ">
			    <div class="input-group pull-right" style="width:400px; text-align: right;">
		                         <!-- <input type="button" id="shanchu1" class="btn btn-success"  value="批量删除" /> --> 
		                        <input type="button" id="peisong1" class="btn btn-success"  value="批量派送" />
		                        
		                    </div>
				<%-- <div class="input-group pull-right" style="width:300px;">
		                        <input type="text" id="searchPhone1" class="form-control" placeholder="请输入电话号码或订单号"   style="width: 200px;"  value="${phone }"/>
		                        <span class="input-group-btn">
		                            <button type="button" class="btn btn-info" onclick="searchPhone1()" >
		                                <i class="glyphicon glyphicon-search"></i>
		                                搜索
		                            </button>
		                        </span>
			...
			<div class="line" style="display:" id="listDiv2">
				<div id="tab2" class="tabson">
					...
					<form id="fenye" name="fenye"action="${pageContext.servletContext.contextPath }/background/order/list.html"
						method="post">
						<table class="tablelist">
							<thead>
								<tr>
								<c:if test="${ state == 1}"><th width="3%"><input id="chose" type="checkbox"	name="checkbox" onclick="selectAllCheckBox()" style="<c:if test='${state!=1 }'>display:none</c:if>"/></th></c:if>	
									<th>订单编号</th>
									<th>发货人姓名</th>
									<th>发货时间</th>
									<th>快递员姓名</th>
							...
							<tbody align="center">
								...
								<c:forEach var="key" items="${te}">
									<tr>
										<c:if test="${ state == 1}"><td><input type="checkbox" name="check" value="${key.id}" style="<c:if test='${state!=1 }'>display:none</c:if>" /></td></c:if>
										...
										<td ><c:choose>
											...
											<td ><c:choose>
												<c:when test="${key.state==0 or key.state==1}">
													<input type="button" class=" btn-primary btn-xs" onclick="update('${key.id}')" value="编辑"  style="<c:if test='${state!=1 and state!=0}'>display:none</c:if>"/>
													&nbsp;
													<c:if test="${ state == 0}">	
														<input type="button" onclick="deletes('${key.id}')" class="  btn-danger btn-xs"  value="删除" style="<c:if test='${state!=1 and state!=0}'>display:none</c:if>"/>
													</c:if>
		                     					 	&nbsp;
		                     					 	<c:if test="${ state == 0}">  
		                     					 		<input type="button" onclick="delivery('${key.id}')" class=" btn-success btn-xs"  value="派送" style="<c:if test='${state!=1 and state!=0}'>display:none</c:if>"/>
		                     					 	</c:if>
												</c:when>
												<c:when test="${ key.state==4}">
													<input type="button" id="map" class="btn-success  btn-xs" onclick="display('${key.id}','${key.courierPhone}','${key.store_longitude}','${key.store_latitude}')" value="显示地图" style="<c:if test='${state!=2  and state!=0}'>display:none</c:if>"/>
												</c:when>
												<c:when test="${key.state==5 or key.state==6}">
			...
			<div class="line" style="display: none;" id="listDiv3">
				<div id="tab3" class="tabson">
					<div class="toolbar1"></div>
					<form id="fenye3" name="fenye"action="${pageContext.servletContext.contextPath }/background/order/list.html"
						method="post">
						<table class="tablelist">
							<thead>
								<tr>
									<th width="3%"><input id="chose" type="checkbox" name="checkbox" onclick="selectAllCheckBox()"/></th>	
									<th>订单编号</th>
									<th>发货人姓名</th>
									<th>发货时间</th>
									<th>快递员姓名</th>
							...
							<tbody align="center">
								<c:choose>
									...
									<tr>
										...
										<td ><p style="color:GREEN ">待维护发货</p></td>
									</tr>
								</c:forEach>
									</c:when>
									<c:otherwise>
						        ...
						        <div class="statistics pull-left">
				                          	 共<b style="color:#337ab7"> ${page1.totalCount} </b>条记录，当前显示<b style="color:#337ab7"> ${page1.pageNo}/${page1.totalPages } </b>页
				                        </div>
				                        <div class="pagination pull-right">
									    	<ul id="page_demo1"></ul>
										</div>
	...
	<script type="text/javascript">
		...
		function   deletes(id){
			if(confirm("确定删除此订单？")){
			 API.ajax("/background/order/deleteLogistics.html",
	   				 {"ids":id}).success(function(result) {
	   					 if(result.state=="ok"){
	   						window.location.href = "${pageContext.request.contextPath}/background/order/list.html?deleteInfo=删除成功!";
	   					 }
		...
		function   delivery(id){
			if(confirm("确定配送此订单？")){
			 API.ajax("/background/order/distribution.html",
	   				 {"ids":id}).success(function(result) {
	   					 if(result.state=="ok"){
	   						window.location.href = "${pageContext.request.contextPath}/background/order/list.html?deleteInfo=配送成功!";
	   					 }
	   			}).error(function(result) {
		 ...
		 $("#peisong").click(function(){
			//判断至少写了一项
	         ...
	         API.ajax("/background/order/distribution.html",
	   				 {"ids":checkedList.toString()}).success(function(result) {
	   					 if(result.state=="ok"){
	   						 alert("配送成功");
		...
		//批量删除
		 $("#shanchu,#shanchu1").click(function(){
					//判断至少写了一项
			         var checkedNum = $("input[name='check']:checked").length;
			         if(checkedNum==0){
			             alert("请至少选择一项!");
