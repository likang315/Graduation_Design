...
public class POIUtils {
	...
	public static List<List<String>> getExcelData(MultipartFile file,
			String fileName, int sheetIndex) throws Exception {
			...
			for (int c = 0; c < totalCells; c++) {
				...
				if (null != cell) {
					...
					case HSSFCell.CELL_TYPE_NUMERIC: // 数字
						...
						String cellStyleStr = cell.getCellStyle()
								.getDataFormatString();
						...
						} else if (cellStyle == 58 || cellStyle == 179
								|| "m\"月\"d\"日\";@".equals(cellStyleStr)) {
	...
	public static List<List<String>> getExcelData(String file, String fileName,
			int sheetIndex) throws Exception {
		List<List<String>> dataLst = new ArrayList<List<String>>();
		Workbook wb = openWorkbook(new FileInputStream(file), fileName);
		Sheet sheet = (Sheet) wb.getSheetAt(sheetIndex);// 切换工作薄
		Row row = null;
		Cell cell = null;
	...
	public static HSSFWorkbook handleDataToExcel(List list, Class clazz,
			String sheetName, int pageSize) throws Exception {
		...
		if (null != list && list.size() > 0) {
			int sheetCount = list.size() % pageSize == 0 ? list.size()
					/ pageSize : list.size() / pageSize + 1;
			for (int i = 1; i <= sheetCount; i++) {
				...
				int end = (begin + pageSize) > list.size() ? list.size()
						: (begin + pageSize);
				int rowCount = 1;
				for (int n = begin; n < end; n++) {
					...
					for (int x = 0; x < headers.size(); x++) {
						...
						Method method = clazz.getDeclaredMethod(headers.get(x)
								.getMethodName());
	...
	public static void exportToExcel(HttpServletResponse response,
			String fileName, List objs, Class clazz, String sheetName,
			int pageSize) {
		...
		try {
			...
			HSSFWorkbook workbook = handleDataToExcel(objs, clazz, "sheet",
					pageSize);
		...
		for (java.lang.reflect.Method m : ms) {
			...
			if (mn.startsWith("get")) {
				if (m.isAnnotationPresent(ExcelDataMapper.class)) {
					ExcelDataMapper dataMapper = m
							.getAnnotation(ExcelDataMapper.class);
					headers.add(new ExcelHeader(dataMapper.title(), dataMapper
							.order(), dataMapper.width(), mn));
				}
			}
		}
		return headers;
